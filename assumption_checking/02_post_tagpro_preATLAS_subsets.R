...
# Load ATLAS files (no headers) - check if they exist first
atlas_files_exist <- file.exists(c(
  "data/clean/post_tagpro/RI_tagger_comparison_ATLAS.csv",
  "data/clean/post_tagpro/PR_tagger_comparison_ATLAS.csv"
))

if (all(atlas_files_exist)) {
  init_tagger_atlas <- read.csv("data/clean/post_tagpro/RI_tagger_comparison_ATLAS.csv", 
                                header = FALSE, stringsAsFactors = FALSE)
  pr_init_runs_atlas <- read.csv("data/clean/post_tagpro/PR_tagger_comparison_ATLAS.csv", 
                                 header = FALSE, stringsAsFactors = FALSE)
  
  cat("Loaded ATLAS files:\n")
  cat("  RI_tagger_comparison_ATLAS.csv:", nrow(init_tagger_atlas), "rows\n")
  cat("  PR_tagger_comparison_ATLAS.csv:", nrow(pr_init_runs_atlas), "rows\n\n")
  
  create_tagger_subsets <- TRUE
} else {
  cat("Note: ATLAS files not found in data/clean/post_tagpro/\n")
  cat("ATLAS files are generated from TagPro output.\n")
  cat("Skipping tagger subset creation.\n\n")
  create_tagger_subsets <- FALSE
}

# ============================================================================
# Create tagger subsets (if ATLAS files exist)
# ============================================================================

if (create_tagger_subsets) {
  ...
  for (tagger in taggers) {
    ...
    init_tagger_file <- file.path(output_dir, paste0("RI_tagger_comparison_ATLAS", suffix, ".csv"))
    write.table(
      init_tagger_subset,
      init_tagger_file,
    ...
    pr_init_runs_subset <- pr_init_runs_atlas |> 
      filter(V3 %in% tag_codes)
    pr_init_runs_file <- file.path(output_dir, paste0("PR_tagger_comparison_ATLAS", suffix, ".csv"))
    write.table(
      pr_init_runs_subset,
      pr_init_runs_file,
      row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE
    ...
    cat("    RI_tagger_comparison_ATLAS:", nrow(init_tagger_subset), "rows\n")
    cat("    PR_tagger_comparison_ATLAS:", nrow(pr_init_runs_subset), "rows\n")
