#' Plot tagger cumulative survival estimates with error bars
#'
#' Creates a ggplot comparing cumulative survival estimates across downstream locations,
#' species, locations, and taggers with error bars representing 95% confidence intervals.
#'
#' @param cumul_cjs_survival A data frame with columns: tagger, location, species,
#'   and columns for each downstream location (e.g., "Release to Crescent Bar_est", "Release to Crescent Bar_se").
#'   Typically from atlas_results$cumul_cjs_survival.
...
#'
#' @details
#' The plot displays cumulative survival estimates by downstream location, faceted by species
#' and location. POOLED results are shown in black with larger points, while individual taggers
#' are shown in distinct colors with smaller points. Cumulative survival represents survival
#' from release to each downstream location.
#'
#' @keywords assumption_check
#' @export
#' @examples
#' \dontrun{
#' plot <- get_tagger_cumul_surv_plot(atlas_results$cumul_cjs_survival)
#' plot
#' }

get_tagger_cumul_surv_plot <- function(cumul_cjs_survival, save_plot = FALSE) {
  ...
  # If input is wide format, convert to long format
  if ("Release to Crescent Bar_est" %in% names(cumul_cjs_survival) || 
      "Release to Lower Ringold_est" %in% names(cumul_cjs_survival)) {
  ...
  reach_names <- c(
    "Release to Crescent Bar" = "Release to\nCrescent Bar",
    "Release to Lower Ringold" = "Release to\nLower Ringold",
    "Release to Sunland" = "Release to\nSunland",
    "Release to Wanapum BRZ" = "Release to\nWanapum BRZ",
    "Release to Mattawa" = "Release to\nMattawa",
    "Release to Priest BRZ" = "Release to\nPriest BRZ"
  )
  
  # Prepare data for visualization
  plot_data <- cumul_cjs_survival |>
    filter(!is.na(tagger)) |>
    mutate(
  ...
  plot <- ggplot(plot_data, aes(x = reach_formatted, y = estimate, 
    ...
    labs(
      title = "Cumulative CJS Survival Estimates by Tagger, Release Location, and Downstream Location",
      x = "Downstream Location",
      y = "Cumulative Survival Estimate",
      fill = "",
      size = ""
    ) +
    theme_gray() +
  ...
  if (save_plot) {
    ...
    # Save as PNG
    png_path <- file.path(output_dir, "tagger_cumul_survival_comparison.png")
